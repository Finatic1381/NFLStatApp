...
def load_and_aggregate_pbp_data(year):
    ...
    defensive_df = reduce(lambda left, right: pd.merge(left, right, on='player_display_name', how='outer'),
                          aggregated_stats)
...
if 'last_refresh' not in st.session_state:
    st.session_state['last_refresh'] = datetime.now().strftime("%B %d, %Y at %I:%M %p")
...
with tab1:
    ...
    if main_category == "Defense":
        st.info(
            "Defensive stats are aggregated from season-long Play-by-Play data and include both regular and post-season.")
        ...
        if main_category == "Offense":
            ...
            ngs_stat_cols = {NGS_TRANSLATIONS.get(col, col): col for col in df_for_leaders.columns if
                             col in NGS_TRANSLATIONS}
...
with tab2:
    ...
    if player_name:
        ...
        if cols_with_data:
            ...
            player_min_week, player_max_week = int(player_weekly_stats['week'].min()), int(
                player_weekly_stats['week'].max())
            player_week_range = st.slider("Select a range of weeks:", player_min_week, player_max_week,
                                          (player_min_week, player_max_week), key="player_week_slider")
            player_filtered_df = player_weekly_stats[(player_weekly_stats['week'] >= player_week_range[0]) & (
                        player_weekly_stats['week'] <= player_week_range[1])]
            ...
            if chart_stat_col:
                fig = px.line(player_weekly_stats, x='week', y=chart_stat_col, title=f"Weekly {chart_stat_key}",
                              markers=True)
